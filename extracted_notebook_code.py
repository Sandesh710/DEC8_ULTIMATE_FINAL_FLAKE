...
C_T_max = np.float64( 0.718282)               # Maximum shape factor
...
C_T = C_T_min  # Use the minimum value (0.5) for initial condition
...
U_a_in = U_wind[0]
e = hum_mb[k] * 100.0
q_a_in = SfcFlx_spechum(e, P_air)
e_a = hum_mb[k] * 100.0
cl_tot = cloud[k]
cl_low = -1.0  # only total cloud
Q_lw_atm = SfcFlx_lwradatm(T_a_in, e_a, cl_tot, cl_low)  # negative downward
Q_atm_lw_in = -Q_lw_atm  # positive downward for FLake bookkeeping
...
rslt_filename = "Heiligensee80-96.rslt"
...
test_file = "Heiligensee80-96.test"
rslt_file = "Heiligensee80-96.rslt"
...
# Files to compare
test_file = "Heiligensee80-96.test"
excel_file = "flake_model_output.xlsx"
detailed_excel = "flake_model_detailed.xlsx"

# Parse Fortran .test file
def parse_fortran_test(filename):
    """Parse Fortran .test output file"""
    ...
    for line in data_lines:
        ...
        if len(parts) >= 22:  # Should have at least 22 columns
            row = []
            for val in parts:
                try:
                    # Convert Fortran scientific notation
                    val_clean = val.replace('E', 'e').replace('D', 'e')
                    row.append(float(val_clean))
                except:
                    row.append(np.nan)
            data.append(row)
    
    # Column names based on FLake output format
    col_names = [
...
# Load data
print(f"\nLoading Fortran .test file:")
df_test = parse_fortran_test(test_file)

print(f"\nLoading Excel files:")
df_excel = pd.read_excel(excel_file)
print(f"   ✓ {excel_file}: {df_excel.shape[0]} rows, {df_excel.shape[1]} columns")

df_detailed = pd.read_excel(detailed_excel)
print(f"   ✓ {detailed_excel}: {df_detailed.shape[0]} rows, {df_detailed.shape[1]} columns")

# Align column names (Excel might have slightly different names)
# Create mapping for common variables
test_columns = set(df_test.columns)
excel_columns = set(df_excel.columns)
common_columns = sorted(list(test_columns.intersection(excel_columns)))

print(f"\n🔍 COMMON VARIABLES FOR COMPARISON ({len(common_columns)}):")
...
COLORS = {
    'test': '#1f77b4',    # Blue for Fortran test
    'excel': '#ff7f0e',   # Orange for Excel output
    'diff': '#d62728',    # Red for differences
...
output_dir = Path("flake_comparison_plots")
...
def calculate_statistics(test_series, excel_series, var_name):
    """Calculate comparison statistics"""
    # Remove NaN values
    mask = ~np.isnan(test_series) & ~np.isnan(excel_series)
    test_clean = test_series[mask]
    excel_clean = excel_series[mask]
    ...
    stats = {
        ...
        'excel_mean': np.mean(excel_clean),
        'test_std': np.std(test_clean),
        'excel_std': np.std(excel_clean),
        'mean_diff': np.mean(test_clean - excel_clean),
        'abs_mean_diff': np.mean(np.abs(test_clean - excel_clean)),
        'max_diff': np.max(np.abs(test_clean - excel_clean)),
        'rmse': np.sqrt(np.mean((test_clean - excel_clean)**2)),
        'correlation': np.corrcoef(test_clean, excel_clean)[0, 1] if len(test_clean) > 1 else np.nan
    }
    
    # Calculate relative difference (avoid division by zero)
    if np.abs(stats['test_mean']) > 1e-10:
        stats['rel_diff_pct'] = (stats['abs_mean_diff'] / np.abs(stats['test_mean'])) * 100
    else:
        stats['rel_diff_pct'] = np.nan
    
    return stats

def create_comparison_plot(time_series, test_data, excel_data, var_name, units="", ax=None):
    """Create a single comparison plot"""
    if ax is None:
        fig, ax = plt.subplots(figsize=(10, 5))
    
    # Plot data
    ax.plot(time_series, test_data, color=COLORS['test'], label='Fortran (.test)', alpha=0.8)
    ax.plot(time_series, excel_data, color=COLORS['excel'], label='Python (Excel)', alpha=0.8, linestyle='--')
    
    # Calculate and plot differences
    diff = test_data - excel_data
    ax2 = ax.twinx()
    ax2.fill_between(time_series, 0, diff, color=COLORS['diff'], alpha=0.3, label='Difference')
    ax2.set_ylabel(f'{var_name} Difference', color=COLORS['diff'], fontsize=10)
    ax2.tick_params(axis='y', labelcolor=COLORS['diff'])
    
    # Style
    ax.set_xlabel('Time (days)', fontsize=11)
    ax.set_ylabel(f'{var_name} ({units})', fontsize=11, color=COLORS['test'])
    ax.set_title(f'{var_name}: Fortran vs Python Comparison', fontsize=13, fontweight='bold', pad=15)
    ...
    stats = calculate_statistics(test_data, excel_data, var_name)
    if stats:
        textstr = '\n'.join([
            f'RMSE: {stats["rmse"]:.3e}',
            f'Corr: {stats["correlation"]:.4f}',
            f'Mean diff: {stats["mean_diff"]:.3e}',
            f'Max diff: {stats["max_diff"]:.3e}'
        ])
...
# Get time series (assume 'time' column exists in both)
if 'time' in df_test.columns and 'time' in df_excel.columns:
    time_series = df_test['time']
else:
    time_series = np.arange(len(df_test))

# Group variables by type for organized plotting
variable_groups = {
    'Temperature (°C)': ['Ts', 'Tm', 'Tb', 'T_ice', 'T_snow', 'T_B1'],
    'Heat Fluxes (W/m²)': ['Qw', 'Q_se', 'Q_la', 'Q_lwa', 'Q_lww', 'Qbot', 'I_w'],
    'Turbulence Parameters': ['ufr_a', 'ufr_w', 'Wconv', 'C_T'],
    'Layer Thicknesses (m)': ['h_ML', 'H_ice', 'H_snow', 'H_B1']
}

# Filter to only include variables present in both datasets
for group_name, vars_list in variable_groups.items():
    variable_groups[group_name] = [v for v in vars_list if v in common_columns]

# ============================================================================
# 4.1 CREATE GROUPED COMPARISON PLOTS (2x2 layout)
# ============================================================================

print("\n1. Creating Grouped Comparison Plots (2x2 grids)...")
...
for group_name, variables in variable_groups.items():
    ...
    if n_vars <= 4:
        # Create 2x2 grid for up to 4 variables
        n_rows = min(2, (n_vars + 1) // 2)
        n_cols = min(2, n_vars)
        
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(14, 4*n_rows))
        fig.suptitle(f'FLake Model Comparison: {group_name}', fontsize=16, fontweight='bold', y=1.02)
        ...
        for idx, var in enumerate(variables[:4]):  # Limit to 4 for clean layout
            ax = axes[idx]
            
            # Get units from group name
            units = group_name.split('(')[-1].strip(')') if '(' in group_name else ""
            
            # Plot comparison
            ax.plot(time_series, df_test[var], color=COLORS['test'], label='Fortran', linewidth=1.5)
            ax.plot(time_series, df_excel[var], color=COLORS['excel'], label='Excel', linewidth=1.5, linestyle='--')
            
            # Add statistics
            stats = calculate_statistics(df_test[var], df_excel[var], var)
            if stats:
                rmse_str = f"RMSE: {stats['rmse']:.2e}" if stats['rmse'] < 1 else f"RMSE: {stats['rmse']:.2f}"
                textstr = f"{rmse_str}\nCorr: {stats['correlation']:.3f}"
                ax.text(0.02, 0.98, textstr, transform=ax.transAxes, fontsize=8,
                       verticalalignment='top',
                       bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
            
            ax.set_xlabel('Time (days)', fontsize=9)
            ax.set_ylabel(f'{var} {units}', fontsize=9)
            ax.set_title(var, fontsize=11, fontweight='bold')
            ax.legend(fontsize=8, loc='upper right')
            ax.grid(True, alpha=0.2)
        
        # Hide empty subplots
        for idx in range(len(variables[:4]), len(axes)):
            axes[idx].axis('off')
        
        plt.tight_layout()
        safe_group_name = group_name.replace('(', '').replace(')', '').replace('/', '').replace(' ', '_')
        plt.savefig(output_dir / f'group_{safe_group_name}.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"   ✓ Saved: group_{safe_group_name}.png")

# ============================================================================
# 4.2 CREATE DETAILED SINGLE-VARIABLE PLOTS
# ============================================================================

print("\n2. Creating Detailed Single-Variable Plots...")

# Select key variables for detailed plots
key_variables = ['Ts', 'Tm', 'Tb', 'h_ML', 'H_ice', 'Qw', 'C_T']
key_variables = [v for v in key_variables if v in common_columns]

for var in key_variables:
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 10), 
                                         gridspec_kw={'height_ratios': [2, 1, 1]})
    fig.suptitle(f'Detailed Analysis: {var}', fontsize=16, fontweight='bold', y=0.98)
    
    # 1. Time series comparison
    ax1.plot(time_series, df_test[var], color=COLORS['test'], label='Fortran (.test)', linewidth=2)
    ax1.plot(time_series, df_excel[var], color=COLORS['excel'], label='Python (Excel)', 
             linewidth=1.5, linestyle='--', alpha=0.9)
    ax1.set_ylabel(var, fontsize=12)
    ax1.set_title(f'Time Series Comparison', fontsize=13)
    ...
    stats = calculate_statistics(df_test[var], df_excel[var], var)
    if stats:
        stat_text = f"RMSE: {stats['rmse']:.3e}\nCorr: {stats['correlation']:.4f}\nMean diff: {stats['mean_diff']:.3e}"
        ax1.text(0.02, 0.98, stat_text, transform=ax1.transAxes, fontsize=9,
                verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
    
    # 2. Difference plot
    diff = df_test[var] - df_excel[var]
    ax2.plot(time_series, diff, color=COLORS['diff'], linewidth=1.5)
    ax2.fill_between(time_series, 0, diff, color=COLORS['diff'], alpha=0.3)
    ax2.axhline(y=0, color='black', linestyle='-', linewidth=0.5, alpha=0.5)
    ax2.set_ylabel(f'{var} Difference', fontsize=12)
    ax2.set_title('Difference (Fortran - Python)', fontsize=13)
    ax2.grid(True, alpha=0.3)
    
    # Add difference statistics
    if stats:
        diff_stats = f"Mean: {np.mean(diff):.3e}\nStd: {np.std(diff):.3e}\nMax: {np.max(np.abs(diff)):.3e}"
        ax2.text(0.02, 0.98, diff_stats, transform=ax2.transAxes, fontsize=9,
                verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
    
    # 3. Scatter plot with 1:1 line
    ax3.scatter(df_test[var], df_excel[var], alpha=0.6, s=10, color='steelblue', edgecolor='none')
    
    # Add 1:1 line
    min_val = min(df_test[var].min(), df_excel[var].min())
    max_val = max(df_test[var].max(), df_excel[var].max())
    ax3.plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=1.5, alpha=0.7, label='1:1 line')
    
    ax3.set_xlabel(f'Fortran {var}', fontsize=12)
    ax3.set_ylabel(f'Python {var}', fontsize=12)
    ax3.set_title('Scatter Plot with 1:1 Line', fontsize=13)
    ax3.legend(loc='upper left', fontsize=10)
    ax3.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / f'detailed_{var}.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"   ✓ Detailed plot: {var}")

# ============================================================================
# 4.3 CREATE STATISTICAL SUMMARY VISUALIZATION
# ============================================================================

print("\n3. Creating Statistical Summary Visualization...")

# Calculate statistics for all common variables
all_stats = []
for var in common_columns:
    if var not in ['No', 'time']:
        stats = calculate_statistics(df_test[var], df_excel[var], var)
        if stats:
...
if all_stats:
    ...
    stats_df = stats_df.sort_values('rmse', ascending=False)
    
    # Create summary figure
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('FLake Model Comparison: Statistical Summary', fontsize=16, fontweight='bold', y=0.98)
    
    # 1. RMSE by variable
    ax1.barh(range(len(stats_df)), stats_df['rmse'], color='skyblue', edgecolor='navy')
    ax1.set_yticks(range(len(stats_df)))
    ax1.set_yticklabels(stats_df['variable'], fontsize=9)
    ax1.set_xlabel('RMSE', fontsize=11)
    ax1.set_title('Root Mean Square Error by Variable', fontsize=13)
    ax1.grid(True, alpha=0.3, axis='x')
    
    # 2. Correlation by variable
    ax2.barh(range(len(stats_df)), stats_df['correlation'], color='lightgreen', edgecolor='darkgreen')
    ax2.set_yticks(range(len(stats_df)))
    ax2.set_yticklabels([])
    ax2.set_xlabel('Correlation Coefficient', fontsize=11)
    ax2.set_title('Correlation by Variable', fontsize=13)
    ax2.axvline(x=0.99, color='red', linestyle='--', alpha=0.7, label='Excellent (0.99)')
    ax2.axvline(x=0.95, color='orange', linestyle='--', alpha=0.7, label='Good (0.95)')
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.3, axis='x')
    
    # 3. Relative difference percentage
    rel_diff_clean = stats_df['rel_diff_pct'].dropna()
    if len(rel_diff_clean) > 0:
        ax3.barh(range(len(rel_diff_clean)), rel_diff_clean, color='lightcoral', edgecolor='darkred')
        ax3.set_yticks(range(len(rel_diff_clean)))
        ax3.set_yticklabels(stats_df.loc[rel_diff_clean.index, 'variable'], fontsize=9)
        ax3.set_xlabel('Relative Difference (%)', fontsize=11)
        ax3.set_title('Relative Difference (%)', fontsize=13)
        ax3.axvline(x=1, color='red', linestyle='--', alpha=0.7, label='1% threshold')
        ax3.axvline(x=5, color='orange', linestyle='--', alpha=0.7, label='5% threshold')
        ax3.legend(fontsize=9)
        ax3.grid(True, alpha=0.3, axis='x')
    
    # 4. Maximum absolute difference
    ax4.barh(range(len(stats_df)), stats_df['max_diff'], color='gold', edgecolor='darkgoldenrod')
    ax4.set_yticks(range(len(stats_df)))
    ax4.set_yticklabels([])
    ax4.set_xlabel('Maximum Absolute Difference', fontsize=11)
    ax4.set_title('Maximum Difference by Variable', fontsize=13)
...
# ============================================================================
# 4.4 CREATE HEATMAP OF DIFFERENCES
# ============================================================================

print("\n4. Creating Difference Heatmap...")

# Create difference matrix
diff_vars = [v for v in common_columns if v not in ['No', 'time']]
diff_matrix = np.zeros((len(diff_vars), len(time_series)))

for i, var in enumerate(diff_vars):
    diff_matrix[i] = df_test[var].values - df_excel[var].values

# Create heatmap
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 8))
fig.suptitle('Difference Heatmap: Fortran (.test) - Python (Excel)', fontsize=16, fontweight='bold', y=0.98)

# 1. Absolute difference heatmap
im1 = ax1.imshow(np.abs(diff_matrix), aspect='auto', cmap='Reds', interpolation='nearest')
ax1.set_title('Absolute Differences', fontsize=13)
ax1.set_xlabel('Time Step', fontsize=11)
ax1.set_ylabel('Variable', fontsize=11)
ax1.set_yticks(range(len(diff_vars)))
ax1.set_yticklabels(diff_vars, fontsize=8)
plt.colorbar(im1, ax=ax1, label='Absolute Difference')

# 2. Relative difference heatmap (normalized by test value range)
rel_diff_matrix = np.zeros_like(diff_matrix)
for i, var in enumerate(diff_vars):
    var_range = df_test[var].max() - df_test[var].min()
    if var_range > 1e-10:
        rel_diff_matrix[i] = np.abs(diff_matrix[i]) / var_range
    else:
        rel_diff_matrix[i] = np.abs(diff_matrix[i])

im2 = ax2.imshow(rel_diff_matrix, aspect='auto', cmap='YlOrRd', interpolation='nearest')
ax2.set_title('Relative Differences (Normalized by Range)', fontsize=13)
ax2.set_xlabel('Time Step', fontsize=11)
ax2.set_ylabel('Variable', fontsize=11)
ax2.set_yticks(range(len(diff_vars)))
ax2.set_yticklabels(diff_vars, fontsize=8)
...
# ============================================================================
# 4.5 CREATE SEASONAL ANALYSIS PLOTS
# ============================================================================

print("\n5. Creating Seasonal Analysis...")

if 'time' in df_test.columns:
    # Extract day of year (assuming data spans multiple years)
    df_test['day_of_year'] = df_test['time'] % 365
    df_excel['day_of_year'] = df_excel['time'] % 365
    
    # Key variables for seasonal analysis
    seasonal_vars = ['Ts', 'Tm', 'Tb', 'h_ML', 'H_ice']
    seasonal_vars = [v for v in seasonal_vars if v in common_columns]
    
    if seasonal_vars:
        n_seasonal = len(seasonal_vars)
        fig, axes = plt.subplots(n_seasonal, 1, figsize=(12, 3*n_seasonal))
        if n_seasonal == 1:
            axes = [axes]
        ...
        for idx, var in enumerate(seasonal_vars):
            ...
            test_seasonal = df_test.groupby('day_of_year')[var].mean()
            excel_seasonal = df_excel.groupby('day_of_year')[var].mean()
            
            # Plot seasonal cycles
            ax.plot(test_seasonal.index, test_seasonal.values, 
                   color=COLORS['test'], label='Fortran', linewidth=2)
            ax.plot(excel_seasonal.index, excel_seasonal.values, 
                   color=COLORS['excel'], label='Excel', linewidth=2, linestyle='--')
            ...
            ax.set_title(f'Seasonal Cycle: {var}', fontsize=13, fontweight='bold')
...
# Save to CSV
stats_file = output_dir / "model_comparison_statistics.csv"
stats_summary.to_csv(stats_file, index=False)

print(f"\n📊 STATISTICAL SUMMARY:")
print(f"   Total variables compared: {len(stats_summary)}")
print(f"   Excellent agreement: {(stats_summary['assessment'] == 'Excellent').sum()}")
print(f"   Very Good agreement: {(stats_summary['assessment'] == 'Very Good').sum()}")
print(f"   Good agreement: {(stats_summary['assessment'] == 'Good').sum()}")
print(f"   Fair agreement: {(stats_summary['assessment'] == 'Fair').sum()}")
print(f"   Poor agreement: {(stats_summary['assessment'] == 'Poor').sum()}")

# Display top 5 variables with largest RMSE
print(f"\n⚠️  TOP 5 VARIABLES WITH LARGEST RMSE:")
top_rmse = stats_summary.nlargest(5, 'rmse')[['variable', 'rmse', 'assessment']]
for _, row in top_rmse.iterrows():
    print(f"   {row['variable']}: RMSE = {row['rmse']:.3e} ({row['assessment']})")

# Display top 5 variables with best correlation
print(f"\n✅ TOP 5 VARIABLES WITH BEST CORRELATION:")
top_corr = stats_summary.nlargest(5, 'correlation')[['variable', 'correlation', 'assessment']]
for _, row in top_corr.iterrows():
    print(f"   {row['variable']}: Correlation = {row['correlation']:.4f} ({row['assessment']})")
...
# Create HTML report
html_report = output_dir / "model_comparison_report.html"

html_content = f"""
<!DOCTYPE html>
<html>
<head>
    <title>FLake Model Comparison Report</title>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .status-fair {{ color: #e74c3c; font-weight: bold; }}
...
<body>
    <div class="container">
        <div class="header">
            <h1>📊 FLake Model Comparison Report</h1>
            <div class="subtitle">
                Fortran (.test) vs Python (Excel) Output Comparison<br>
        ...
        <div class="summary-box">
            ...
            <ul>
                <li>Fortran Reference: {test_file} ({len(df_test)} rows, {len(df_test.columns)} columns)</li>
                <li>Python Output: {excel_file} ({len(df_excel)} rows, {len(df_excel.columns)} columns)</li>
            </ul>
            <p><strong>Comparison Period:</strong> {df_test['time'].min():.1f} to {df_test['time'].max():.1f} days</p>
            <p><strong>Variables Compared:</strong> {len(common_columns)} common variables</p>
        </div>
...
# Add all generated plots to HTML
...
# Add statistics table rows
for _, row in stats_summary.iterrows():
    # Determine CSS class for assessment
    if row['assessment'] == 'Excellent':
        status_class = 'status-excellent'
    elif row['assessment'] in ['Very Good', 'Good']:
        status_class = 'status-good'
    elif row['assessment'] == 'Fair':
...
# ============================================================================
# 1. LOAD FLake OUTPUT DATA
# ============================================================================

print("🌊 LAKE TEMPERATURE PROFILE VISUALIZATION")
print("="*80)

# Load the FLake output data
excel_file = "flake_model_output.xlsx"
detailed_excel = "flake_model_detailed.xlsx"

print(f"Loading FLake output files...")
df = pd.read_excel(excel_file)
df_detailed = pd.read_excel(detailed_excel)

print(f"   Main output: {df.shape[0]} time steps, {df.shape[1]} variables")
print(f"   Detailed output: {df_detailed.shape[0]} time steps, {df_detailed.shape[1]} variables")

# Get lake depth from configuration (you'll need to adjust this based on your NML)
LAKE_DEPTH = 5.9  # Example depth in meters - UPDATE THIS WITH YOUR ACTUAL DEPTH
print(f"\nAssuming lake depth: {LAKE_DEPTH} m (update if different)")

# ============================================================================
# 2. FLake TEMPERATURE PROFILE RECONSTRUCTION
# ============================================================================

def reconstruct_flake_temperature_profile(T_surface, T_bottom, h_ML, C_T, lake_depth, n_points=100):
    """
    Reconstruct temperature profile from FLake parameters.
    
    FLake uses a two-layer representation:
    - Mixed layer (0 to h_ML): constant temperature = T_surface
    - Thermocline (h_ML to lake_depth): parabolic profile
    
    The shape factor C_T determines the curvature of the thermocline.
    ...
    h_ML : float
        Mixed layer depth (m)
    C_T : float
        Shape factor (0.5 = linear, >0.5 = parabolic)
    lake_depth : float
        Total lake depth (m)
    n_points : int
        Number of depth points for reconstruction
    ...
    # Create depth array
    depths = np.linspace(0, lake_depth, n_points)
    temperatures = np.zeros_like(depths)
    
    # Handle edge cases
    if h_ML <= 0:
        h_ML = 0.001  # Small value to avoid division by zero
    
    if h_ML >= lake_depth:
        # Entire lake is mixed
        temperatures[:] = T_surface
        return depths, temperatures
    
    # Mixed layer (constant temperature)
    mixed_mask = depths <= h_ML
    temperatures[mixed_mask] = T_surface
    
    # Thermocline region (parabolic profile)
    thermocline_mask = depths > h_ML
    
    if C_T <= 0.5:
        # Linear profile for C_T = 0.5
        z_norm = (depths[thermocline_mask] - h_ML) / (lake_depth - h_ML)
        temperatures[thermocline_mask] = T_surface + (T_bottom - T_surface) * z_norm
    else:
        # Parabolic profile for C_T > 0.5
        # FLake uses: T(z) = T_surface + (T_bottom - T_surface) * ((z - h_ML)/(H - h_ML))^(1/C_T)
        z_norm = (depths[thermocline_mask] - h_ML) / (lake_depth - h_ML)
        temperatures[thermocline_mask] = T_surface + (T_bottom - T_surface) * (z_norm ** (1/C_T))
    
    return depths, temperatures

def calculate_thermal_structure_metrics(depths, temperatures, lake_depth):
    """
    Calculate thermal structure metrics.
    
    Returns:
    --------
    metrics : dict
        Dictionary of thermal structure metrics
    """
    # Find thermocline depth (maximum temperature gradient)
    temp_gradient = np.gradient(temperatures, depths)
    if np.any(~np.isnan(temp_gradient)):
        # Find depth of maximum gradient (thermocline)
        max_grad_idx = np.nanargmax(np.abs(temp_gradient))
        thermocline_depth = depths[max_grad_idx]
        max_gradient = temp_gradient[max_grad_idx]
    else:
        thermocline_depth = np.nan
        max_gradient = np.nan
    
    # Calculate thermal stratification index
    # (difference between surface and bottom)
    stratification_index = temperatures[0] - temperatures[-1]
    
    # Calculate Schmidt stability (simplified)
    # Reference temperature (mean)
    T_mean = np.nanmean(temperatures)
    # Density difference (simplified using linear expansion)
    alpha = 2.0e-4  # Thermal expansion coefficient for water (1/°C)
    density_diff = alpha * (temperatures - T_mean)
    # Integrate over depth
    stability = np.trapz(density_diff * depths, depths)
...
output_dir = Path("lake_temperature_profiles")
output_dir.mkdir(exist_ok=True)
...
# ============================================================================
# 4. VISUALIZE TEMPERATURE PROFILES AT KEY TIMES
# ============================================================================

print("\n" + "="*80)
print("📊 CREATING TEMPERATURE PROFILE VISUALIZATIONS")
print("="*80)

# Get time array
if 'time' in df.columns:
    time_days = df['time'].values
elif 'time_days' in df_detailed.columns:
    time_days = df_detailed['time_days'].values
else:
    time_days = np.arange(len(df))

# Define key time points for visualization
# 1. Start of simulation
# 2. Summer stratification (warmest surface)
# 3. Fall turnover (when Ts ≈ Tb)
# 4. Winter (coldest period)
# 5. End of simulation

key_times = []
key_labels = []

# Time 1: Start (index 0)
key_times.append(0)
key_labels.append("Start")

# Time 2: Warmest surface temperature
if 'Ts' in df.columns:
    warmest_idx = df['Ts'].idxmax()
    key_times.append(warmest_idx)
    key_labels.append(f"Summer (Day {int(time_days[warmest_idx])})")

# Time 3: Minimum stratification (Ts closest to Tb)
if 'Ts' in df.columns and 'Tb' in df.columns:
    stratification = np.abs(df['Ts'] - df['Tb'])
    min_strat_idx = stratification.idxmin()
    key_times.append(min_strat_idx)
    key_labels.append(f"Turnover (Day {int(time_days[min_strat_idx])})")

# Time 4: Coldest surface temperature
if 'Ts' in df.columns:
    coldest_idx = df['Ts'].idxmin()
    key_times.append(coldest_idx)
    key_labels.append(f"Winter (Day {int(time_days[coldest_idx])})")

# Time 5: End
key_times.append(len(df) - 1)
key_labels.append("End")

print(f"\nAnalyzing {len(key_times)} key time points:")

# Create figure for all profiles
fig_all, ax_all = plt.subplots(figsize=(10, 8))

# Define color palette for different seasons
season_colors = plt.cm.viridis(np.linspace(0, 1, len(key_times)))

# Store metrics for all key times
all_metrics = []

for i, (time_idx, label) in enumerate(zip(key_times, key_labels)):
    print(f"\n  {label}:")
    
    # Extract FLake variables at this time
    try:
        T_surface = df.loc[time_idx, 'Ts'] if 'Ts' in df.columns else df_detailed.loc[time_idx, 'T_sfc_C']
        T_bottom = df.loc[time_idx, 'Tb'] if 'Tb' in df.columns else df_detailed.loc[time_idx, 'T_bot_C']
        h_ML = df.loc[time_idx, 'h_ML'] if 'h_ML' in df.columns else df_detailed.loc[time_idx, 'h_ML']
        C_T = df.loc[time_idx, 'C_T'] if 'C_T' in df.columns else df_detailed.loc[time_idx, 'C_T']
        time_val = time_days[time_idx]
    except KeyError as e:
        print(f"    ⚠️  Missing variable: {e}")
        continue
    
    # Reconstruct temperature profile
    depths, temperatures = reconstruct_flake_temperature_profile(
        T_surface, T_bottom, h_ML, C_T, LAKE_DEPTH, n_points=200
    )
    
    # Calculate thermal metrics
    metrics = calculate_thermal_structure_metrics(depths, temperatures, LAKE_DEPTH)
    metrics['time_idx'] = time_idx
    metrics['time_day'] = time_val
    metrics['label'] = label
    all_metrics.append(metrics)
    
    # Plot individual profile
    fig_ind, ax_ind = plt.subplots(figsize=(8, 10))
    
    # Temperature profile
    ax_ind.plot(temperatures, depths, color='red', linewidth=2.5, label='Temperature')
    ax_ind.fill_betweenx(depths, 0, temperatures, color='red', alpha=0.2)
    
    # Add mixed layer depth line
    ax_ind.axhline(y=h_ML, color='blue', linestyle='--', linewidth=1.5, 
                  alpha=0.7, label=f'Mixed Layer Depth: {h_ML:.2f} m')
    
    # Add thermocline depth if identified
    if not np.isnan(metrics['thermocline_depth']):
        ax_ind.axhline(y=metrics['thermocline_depth'], color='green', 
                      linestyle=':', linewidth=1.5, alpha=0.7,
                      label=f'Thermocline: {metrics["thermocline_depth"]:.2f} m')
    
    # Style individual plot
    ax_ind.set_ylim(LAKE_DEPTH, 0)  # Invert y-axis (0 at top)
    ax_ind.set_xlabel('Temperature (°C)', fontsize=12)
    ax_ind.set_ylabel('Depth (m)', fontsize=12)
    ax_ind.set_title(f'Lake Temperature Profile: {label}\nDay {int(time_val)}', 
                    fontsize=14, fontweight='bold', pad=15)
    ax_ind.grid(True, alpha=0.3)
    ax_ind.legend(loc='lower right', fontsize=10, framealpha=0.9)
    
    # Add info text box
    info_text = (
        f"Surface: {T_surface:.2f}°C\n"
        f"Bottom: {T_bottom:.2f}°C\n"
        f"ΔT: {metrics['stratification_index']:.2f}°C\n"
        f"Shape factor C_T: {C_T:.3f}\n"
        f"Mean temp: {metrics['mean_temp']:.2f}°C"
    )
    ax_ind.text(0.02, 0.98, info_text, transform=ax_ind.transAxes, fontsize=10,
                verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9, edgecolor='gray'))
    
    plt.tight_layout()
    safe_label = label.replace(' ', '_').replace('(', '').replace(')', '').replace(':', '')
    plt.savefig(output_dir / f'temperature_profile_{safe_label}.png', dpi=300)
    plt.close(fig_ind)
    
    print(f"    ✓ Surface: {T_surface:.2f}°C, Bottom: {T_bottom:.2f}°C")
    print(f"      Mixed layer: {h_ML:.2f} m, Stratification: {metrics['stratification_index']:.2f}°C")
    
    # Add to combined plot
    ax_all.plot(temperatures, depths, color=season_colors[i], 
                linewidth=2, alpha=0.8, label=f'{label} (Day {int(time_val)})')

# Style combined plot
ax_all.set_ylim(LAKE_DEPTH, 0)  # Invert y-axis
ax_all.set_xlabel('Temperature (°C)', fontsize=13)
ax_all.set_ylabel('Depth (m)', fontsize=13)
ax_all.set_title('Lake Temperature Profiles: Seasonal Evolution', 
                fontsize=16, fontweight='bold', pad=20)
ax_all.grid(True, alpha=0.2)
ax_all.legend(loc='best', fontsize=10, framealpha=0.9)

# Add lake bottom line
ax_all.axhline(y=LAKE_DEPTH, color='black', linestyle='-', linewidth=2, alpha=0.5)

plt.tight_layout()
plt.savefig(output_dir / 'all_temperature_profiles.png', dpi=300)
plt.close(fig_all)

print(f"\n✓ Saved individual profiles and combined plot")

# ============================================================================
# 5. CREATE TEMPERATURE CONTOUR PLOT (HEATMAP OVER TIME)
# ============================================================================

print("\n5. Creating Temperature Contour Plot (Heatmap)...")

# Create regular time-depth grid
n_time_points = min(100, len(df))  # Limit to 100 time points for clarity
n_depth_points = 100

# Select evenly spaced time indices
time_indices = np.linspace(0, len(df)-1, n_time_points, dtype=int)
selected_times = time_days[time_indices]

# Initialize temperature matrix
temp_matrix = np.zeros((n_depth_points, n_time_points))

# Reconstruct profiles for each time
for j, time_idx in enumerate(time_indices):
    try:
        ...
        depths, temperatures = reconstruct_flake_temperature_profile(
            T_surface, T_bottom, h_ML, C_T, LAKE_DEPTH, n_points=n_depth_points
        )
        
        temp_matrix[:, j] = temperatures
    except Exception as e:
        print(f"  Warning at time {j}: {e}")
        temp_matrix[:, j] = np.nan

# Create contour plot
fig_contour, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 12), 
                                       gridspec_kw={'height_ratios': [3, 1]})

# Contour plot
X, Y = np.meshgrid(selected_times, depths)
contour = ax1.contourf(X, Y, temp_matrix, levels=50, cmap='RdYlBu_r')
plt.colorbar(contour, ax=ax1, label='Temperature (°C)')

# Overlay mixed layer depth
if 'h_ML' in df.columns:
    mixed_layer_depths = df.loc[time_indices, 'h_ML'].values
    ax1.plot(selected_times, mixed_layer_depths, 'w--', linewidth=2, label='Mixed Layer Depth')
    
    # Fill mixed layer region
    ax1.fill_between(selected_times, 0, mixed_layer_depths, color='white', alpha=0.2)

# Overlay 4°C isotherm (temperature of maximum density)
ax1.contour(X, Y, temp_matrix, levels=[4], colors=['blue'], linewidths=2, linestyles='-')

ax1.set_ylim(LAKE_DEPTH, 0)  # Invert y-axis
ax1.set_xlabel('Time (days)', fontsize=12)
ax1.set_ylabel('Depth (m)', fontsize=12)
ax1.set_title('Lake Temperature Evolution: Depth-Time Contour', 
             fontsize=16, fontweight='bold', pad=20)
ax1.grid(True, alpha=0.2, color='white')
ax1.legend(loc='upper right', fontsize=10, framealpha=0.9)

# Add ice cover if present
if 'H_ice' in df.columns:
    ice_thickness = df.loc[time_indices, 'H_ice'].values * 100  # Convert to cm
    ice_mask = ice_thickness > 0.1  # Ice present if > 1 mm
    
    if np.any(ice_mask):
        ax1.fill_between(selected_times[ice_mask], -ice_thickness[ice_mask]/100, 0, 
                        color='cyan', alpha=0.5, label='Ice Cover')
        ax1.set_ylim(LAKE_DEPTH, -np.max(ice_thickness)/100)  # Extend above surface for ice

# Bottom plot: Surface and bottom temperature time series
ax2.plot(selected_times, temp_matrix[0, :], 'r-', linewidth=2, label='Surface Temperature')
ax2.plot(selected_times, temp_matrix[-1, :], 'b-', linewidth=2, label='Bottom Temperature')
ax2.axhline(y=4, color='blue', linestyle=':', alpha=0.5, label='4°C (Max Density)')

ax2.set_xlabel('Time (days)', fontsize=12)
ax2.set_ylabel('Temperature (°C)', fontsize=12)
ax2.set_title('Surface and Bottom Temperature Time Series', 
             fontsize=14, fontweight='bold', pad=15)
ax2.grid(True, alpha=0.3)
ax2.legend(loc='best', fontsize=10, framealpha=0.9)

plt.tight_layout()
plt.savefig(output_dir / 'temperature_contour_evolution.png', dpi=300)
plt.close(fig_contour)

print("✓ Saved temperature contour plot")

# ============================================================================
# 6. CREATE 3D TEMPERATURE PROFILE ANIMATION (OPTIONAL)
# ============================================================================

print("\n6. Creating 3D Temperature Profile Visualization...")
...
try:
    ...
    # Plot selected profiles in 3D
    n_profiles = 20  # Number of profiles to show
    profile_indices = np.linspace(0, len(df)-1, n_profiles, dtype=int)
    
    colors = plt.cm.viridis(np.linspace(0, 1, n_profiles))
    
    for i, time_idx in enumerate(profile_indices):
        try:
            T_surface = df.loc[time_idx, 'Ts'] if 'Ts' in df.columns else df_detailed.loc[time_idx, 'T_sfc_C']
            T_bottom = df.loc[time_idx, 'Tb'] if 'Tb' in df.columns else df_detailed.loc[time_idx, 'T_bot_C']
            h_ML = df.loc[time_idx, 'h_ML'] if 'h_ML' in df.columns else df_detailed.loc[time_idx, 'h_ML']
            ...
            depths, temperatures = reconstruct_flake_temperature_profile(
                T_surface, T_bottom, h_ML, C_T, LAKE_DEPTH, n_points=50
            )
            
            # Create time array for this profile
            time_array = np.full_like(depths, time_days[time_idx])
            
            # Plot profile
            ax_3d.plot(time_array, temperatures, depths, 
                      color=colors[i], linewidth=1.5, alpha=0.8)
            
            # Mark surface and bottom
            ax_3d.scatter([time_days[time_idx]], [T_surface], [0], 
                         color='red', s=20, alpha=0.8)
            ax_3d.scatter([time_days[time_idx]], [T_bottom], [LAKE_DEPTH], 
    ...
    # Style 3D plot
    ax_3d.set_xlabel('Time (days)', fontsize=11, labelpad=10)
    ax_3d.set_ylabel('Temperature (°C)', fontsize=11, labelpad=10)
    ax_3d.set_zlabel('Depth (m)', fontsize=11, labelpad=10)
    ax_3d.set_title('3D Lake Temperature Evolution', 
                   fontsize=16, fontweight='bold', pad=20)
    
    # Invert z-axis (depth)
    ax_3d.set_zlim(LAKE_DEPTH, 0)
    
    # Set view angle
    ax_3d.view_init(elev=25, azim=-60)
    
    plt.tight_layout()
    plt.savefig(output_dir / '3d_temperature_evolution.png', dpi=300)
    plt.close(fig_3d)
    
    print("✓ Saved 3D temperature visualization")
    
except ImportError:
    print("⚠️  3D plotting requires mpl_toolkits, skipping 3D visualization")

# ============================================================================
# 7. CREATE THERMAL METRICS TIME SERIES
# ============================================================================

print("\n7. Creating Thermal Metrics Time Series...")

# Calculate thermal metrics for all time steps (sampled)
n_samples = min(200, len(df))
sample_indices = np.linspace(0, len(df)-1, n_samples, dtype=int)

# Initialize arrays for metrics
thermal_metrics = {
    'time': time_days[sample_indices],
    'stratification': np.zeros(n_samples),
    'thermocline_depth': np.zeros(n_samples),
    'stability': np.zeros(n_samples),
    'surface_temp': np.zeros(n_samples),
    'bottom_temp': np.zeros(n_samples),
    'mixed_layer_depth': np.zeros(n_samples)
}
...
for i, time_idx in enumerate(sample_indices):
    try:
        ...
        depths, temperatures = reconstruct_flake_temperature_profile(
            T_surface, T_bottom, h_ML, C_T, LAKE_DEPTH, n_points=100
        )
        
        metrics = calculate_thermal_structure_metrics(depths, temperatures, LAKE_DEPTH)
        
        thermal_metrics['stratification'][i] = metrics['stratification_index']
        thermal_metrics['thermocline_depth'][i] = metrics['thermocline_depth']
        thermal_metrics['stability'][i] = metrics['stability']
        thermal_metrics['surface_temp'][i] = T_surface
        thermal_metrics['bottom_temp'][i] = T_bottom
        thermal_metrics['mixed_layer_depth'][i] = h_ML
        
    except Exception as e:
        thermal_metrics['stratification'][i] = np.nan
        thermal_metrics['thermocline_depth'][i] = np.nan
        thermal_metrics['stability'][i] = np.nan
        thermal_metrics['surface_temp'][i] = np.nan
        thermal_metrics['bottom_temp'][i] = np.nan
        thermal_metrics['mixed_layer_depth'][i] = np.nan

# Plot thermal metrics
fig_metrics, axes = plt.subplots(3, 2, figsize=(14, 12))
axes = axes.flatten()

# 1. Stratification index
ax = axes[0]
ax.plot(thermal_metrics['time'], thermal_metrics['stratification'], 'b-', linewidth=1.5)
ax.fill_between(thermal_metrics['time'], 0, thermal_metrics['stratification'], 
                where=thermal_metrics['stratification']>0, color='blue', alpha=0.3)
ax.fill_between(thermal_metrics['time'], 0, thermal_metrics['stratification'], 
                where=thermal_metrics['stratification']<0, color='red', alpha=0.3)
ax.axhline(y=0, color='black', linestyle='-', linewidth=0.5, alpha=0.5)
ax.set_xlabel('Time (days)', fontsize=10)
ax.set_ylabel('Stratification (ΔT, °C)', fontsize=10)
ax.set_title('Thermal Stratification Index', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.2)

# 2. Thermocline depth
ax = axes[1]
ax.plot(thermal_metrics['time'], thermal_metrics['thermocline_depth'], 'g-', linewidth=1.5)
ax.set_xlabel('Time (days)', fontsize=10)
ax.set_ylabel('Thermocline Depth (m)', fontsize=10)
ax.set_title('Thermocline Depth Evolution', fontsize=12, fontweight='bold')
ax.set_ylim(LAKE_DEPTH, 0)  # Invert y-axis
ax.grid(True, alpha=0.2)

# 3. Mixed layer depth
ax = axes[2]
ax.plot(thermal_metrics['time'], thermal_metrics['mixed_layer_depth'], 'purple', 
        linewidth=1.5, label='Mixed Layer')
ax.plot(thermal_metrics['time'], thermal_metrics['thermocline_depth'], 'g--', 
        linewidth=1, alpha=0.7, label='Thermocline')
ax.set_xlabel('Time (days)', fontsize=10)
ax.set_ylabel('Depth (m)', fontsize=10)
ax.set_title('Mixed Layer and Thermocline Depths', fontsize=12, fontweight='bold')
ax.set_ylim(LAKE_DEPTH, 0)  # Invert y-axis
...
# 4. Stability index
ax = axes[3]
ax.plot(thermal_metrics['time'], thermal_metrics['stability'], 'orange', linewidth=1.5)
ax.axhline(y=0, color='black', linestyle='-', linewidth=0.5, alpha=0.5)
ax.set_xlabel('Time (days)', fontsize=10)
ax.set_ylabel('Stability Index', fontsize=10)
ax.set_title('Thermal Stability', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.2)

# 5. Surface vs Bottom temperature
ax = axes[4]
ax.plot(thermal_metrics['time'], thermal_metrics['surface_temp'], 'r-', 
        linewidth=1.5, label='Surface')
ax.plot(thermal_metrics['time'], thermal_metrics['bottom_temp'], 'b-', 
        linewidth=1.5, label='Bottom')
ax.axhline(y=4, color='blue', linestyle=':', alpha=0.5, label='4°C')
ax.set_xlabel('Time (days)', fontsize=10)
ax.set_ylabel('Temperature (°C)', fontsize=10)
ax.set_title('Surface and Bottom Temperature', fontsize=12, fontweight='bold')
ax.legend(fontsize=9)
ax.grid(True, alpha=0.2)

# 6. Temperature difference histogram
ax = axes[5]
temp_diff = thermal_metrics['surface_temp'] - thermal_metrics['bottom_temp']
ax.hist(temp_diff[~np.isnan(temp_diff)], bins=30, color='teal', alpha=0.7, edgecolor='black')
ax.axvline(x=0, color='red', linestyle='--', linewidth=1.5, alpha=0.7)
ax.set_xlabel('Surface - Bottom Temperature (°C)', fontsize=10)
ax.set_ylabel('Frequency', fontsize=10)
ax.set_title('Temperature Difference Distribution', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.2)

plt.tight_layout()
plt.savefig(output_dir / 'thermal_metrics_time_series.png', dpi=300)
plt.close(fig_metrics)

print("✓ Saved thermal metrics time series")
...
print("\n" + "="*80)
print("📋 TEMPERATURE PROFILE ANALYSIS SUMMARY")
print("="*80)

print(f"\n🌊 Lake Characteristics:")
print(f"   • Assumed depth: {LAKE_DEPTH} m")
print(f"   • Simulation period: {time_days[0]:.0f} to {time_days[-1]:.0f} days")
print(f"   • Number of time steps analyzed: {len(df)}")

print(f"\n📊 Key Thermal Metrics (from sampled profiles):")
if all_metrics:
    print(f"   • Maximum stratification: {np.nanmax(thermal_metrics['stratification']):.2f}°C")
    print(f"   • Minimum stratification: {np.nanmin(thermal_metrics['stratification']):.2f}°C")
    print(f"   • Average mixed layer depth: {np.nanmean(thermal_metrics['mixed_layer_depth']):.2f} m")
    print(f"   • Shallowest thermocline: {np.nanmin(thermal_metrics['thermocline_depth']):.2f} m")
    print(f"   • Deepest thermocline: {np.nanmax(thermal_metrics['thermocline_depth']):.2f} m")

print(f"\n❄️  Ice Cover Analysis:")
if 'H_ice' in df.columns:
    ice_days = (df['H_ice'] > 0.01).sum()  # Days with > 1 cm ice
    max_ice = df['H_ice'].max() * 100  # Convert to cm
    print(f"   • Ice cover present on {ice_days} days ({ice_days/len(df)*100:.1f}% of time)")
    print(f"   • Maximum ice thickness: {max_ice:.1f} cm")

print(f"\n💾 Output Files Saved to {output_dir}/:")
print(f"   1. temperature_profile_*.png - Individual profiles at key times")
print(f"   2. all_temperature_profiles.png - Combined seasonal profiles")
print(f"   3. temperature_contour_evolution.png - Depth-time contour plot")
print(f"   4. thermal_metrics_time_series.png - Thermal structure metrics")
if '3d_temperature_evolution.png' in [f.name for f in output_dir.glob('*.png')]:
    print(f"   5. 3d_temperature_evolution.png - 3D visualization")

print("\n" + "="*80)
print("✅ Temperature profile analysis complete!")
print("="*80)

# Save metrics to CSV
metrics_df = pd.DataFrame(thermal_metrics)
metrics_df.to_csv(output_dir / 'thermal_metrics.csv', index=False)
print(f"\n📄 Thermal metrics saved to: {output_dir}/thermal_metrics.csv")
